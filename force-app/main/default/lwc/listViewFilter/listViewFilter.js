...
export default class ListViewFilter extends LightningElement {
    ...
    @api fullData;
    @api pricebooklist;
    @api priceBookvalFetch;
    @api priceBookLabelValFetch;
    @track priceBook;
    @track pricebookVal;
    @track priceBookLabelVal;
    ...
    @track dataList = [];
    getFieldList() {
        this.filterFieldList = JSON.parse(JSON.stringify(this.filterListValue));
        this.priceBookProductList = JSON.parse(JSON.stringify(this.pricebooklist));
        this.priceBookLabelVal = JSON.parse(JSON.stringify(this.priceBookLabelValFetch));
        this.pricebookVal = JSON.parse(JSON.stringify(this.priceBookvalFetch));
        ...
        for (var key in data.fieldMap.Product) {
            if (key != 'Id') {
                fields.push({

                    label: data.fieldMap.Product[key].split(',')[0],
                    value: key,
                    type: data.fieldMap.Product[key].split(',')[1],
                });
            }
        }
    ...
    handleSaveFilter() {
        ...
        const filterValue = new CustomEvent("getfiltervalue", { detail: recordsToCheck });
